...
class get_ROI_and_MatchInfo(object):
    ...
    def __call__(self, target_a, target_b, noise=None, shape=None):
        ...
        roi_a[:, 3] = torch.clamp(gt_a[:, 0] + self.radius, max=self.w-1)
        roi_a[:, 4] = torch.clamp(gt_a[:, 1] + self.radius, max=self.h-1)

        roi_b[:, 0] = 1
        roi_b[:, 1] = torch.clamp(gt_b[:, 0] - self.radius, min=0)
        roi_b[:, 2] = torch.clamp(gt_b[:, 1] - self.radius, min=0)
        roi_b[:, 3] = torch.clamp(gt_b[:, 0] + self.radius, max=self.w-1)
        roi_b[:, 4] = torch.clamp(gt_b[:, 1] + self.radius, max=self.h-1)
